...
class LitModel(pl.LightningModule):
    ...
    def score_fn(self, x, t):
        ...
        return -noise/self.sampler.posterior_variance

    def encode(self, x):
        # TODO:
        assert self.conf.model_type.has_autoenc()
        encoder = self.ema_model.encoder
    ...
    def log_density_fn(self, x, z, t):
        latent_distribution_parameters = self.ema_model.encoder(x, t)
        channels = latent_distribution_parameters.size(1) // 2
        mean_z = latent_distribution_parameters[:, :channels]
        log_var_z = latent_distribution_parameters[:, channels:]
    ...
    def correction_score(self, x_start, z, t):
        ftx = self.log_density_fn(x_start, z, t)
        return torch.autograd.grad(outputs=ftx, inputs=x,
                                grad_outputs=torch.ones(ftx.size()).to(x_start.device),
    ...
    def training_step(self, batch, batch_idx):
        ...
        with amp.autocast(False):
            ...
            if self.conf.train_mode == TrainMode.diffusion:
                ...
                # losses = self.sampler.training_losses(model=self.correction_score,
                #                                       x_start=x_start,
                #                                       t=t,)
                losses = 
...
def train(conf: TrainConfig, gpus, nodes=1, mode: str = 'train'):
    ...
    model = LitModel(conf)
    model.sampler.condition_mean = model.correction_score
